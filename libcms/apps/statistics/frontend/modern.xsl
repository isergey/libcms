...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
                ...
                <xsl:choose>
                    <xsl:when test="@AllowChange and $MultiLevel!='true'">
                        <select class="stat-parameters-select">
                            <xsl:for-each
                                    select="/Reports/Dimensions/Dimension[@Code=$DimensionCode]/Element[@ReportUrl]">
                                <option>
                        ...
                        <xsl:if test="/Reports/Dimensions/Dimension[@Code=$DimensionCode]/@MaxLevel > 1">
                            <xsl:call-template name="PrintLevelControls">
                                <xsl:with-param name="MaxLevel"
                                                select="/Reports/Dimensions/Dimension[@Code=$DimensionCode]/@MaxLevel"/>
                            </xsl:call-template>
                        </xsl:if>
    ...
    <xsl:template name="PrintMultiLevelParameter">
        ...
        <ul>
            <xsl:for-each
                    select="$Dimension/Element[@ReportUrl and (Parent/@Code=$ElementCode or (count(Parent/@Code)=0 and not($ElementCode)))]">
                <li>
                ...
                <xsl:if test="$rowNum=1 and count(../../../TableHeader[@Type='Vertical']/Header)>0">
                    ...
                    <th class="stat-level-1">
                        ...
                        <xsl:attribute name="RowSpan">
                            <xsl:value-of
                                    select="count(../../../TableHeader[@Type='Horizontal']/Header[position()=1]/Element)"/>
                        </xsl:attribute>
    ...
    <xsl:template name="PrintHeaderElement">
        ...
        <xsl:variable name="ReportUrl2"
                      select="/Reports/Dimensions/Dimension[@Code=$DimensionCode]/Element[@Code=$ElementCode]/@ReportUrl"/>
    ...
    <xsl:template name="PrintName">
        ...
        <xsl:variable name="name"
                      select="/Reports/Dimensions/Dimension[@Code=$DimensionCode]/Element[@Code=$ElementCode]/@*[name()=$nameColumn]"/>
        <xsl:choose>
    ...
    <xsl:template name="PrintFullName">
        ...
        <xsl:for-each
                select="/Reports/Dimensions/Dimension[@Code=$DimensionCode]/Element[@Code=$ElementCode]/Parent2/@Code">
            <div>
                <xsl:call-template name="PrintName">
                    <xsl:with-param name="DimensionCode" select="$DimensionCode"/>
        ...
        <xsl:if test="/Reports/Dimensions/Dimension[@Code=$DimensionCode]/Element[@Code=$ElementCode]/Parent/@Code">
            <div>
                <xsl:call-template name="PrintName">
                    <xsl:with-param name="DimensionCode" select="$DimensionCode"/>
                    <xsl:with-param name="ElementCode"
                                    select="/Reports/Dimensions/Dimension[@Code=$DimensionCode]/Element[@Code=$ElementCode]/Parent/@Code"/>
                </xsl:call-template>
            </div>
        </xsl:if>
        <div>
