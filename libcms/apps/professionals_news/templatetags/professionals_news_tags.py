...
@register.inclusion_tag('professionals_news/tags/news_feed.html')
def professionals_news_feed(count=5):
    news_list = list(News.objects.filter(publicated=True).order_by('-create_date')[:count])
    lang=get_language()[:2]
